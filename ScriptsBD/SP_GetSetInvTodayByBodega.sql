...
GO
IF OBJECT_ID('SP_GetSetInvTodayByBodega', 'P') IS NOT NULL
	DROP PROC dbo.SP_GetSetInvTodayByBodega
GO
CREATE PROCEDURE dbo.SP_GetSetInvTodayByBodega
@ClienteId int,
@CodUser VARCHAR(128)
AS
BEGIN
	SET @CodUser = @CodUser + 'BOT'
	...
	SELECT DISTINCT
		...
		CONVERT(VARCHAR(8), wpe.BodegaId),
		D.Lote,
		D.Departamento,
		2 Typ,
		@CodUser
	...
	SELECT 
		CONVERT(INT, D.Talla) BodegaId,
		(SELECT B.NomBodega FROM wms.dbo.Bodegas B where B.BodegaID = CONVERT(INT, D.Talla)) Bodega,
		CONVERT(INT, D.Producto) TiendaId,
		(SELECT Ti.Descr FROM EdiDB.dbo.PAYLESS_Tiendas Ti WHERE Ti.TiendaId = CONVERT(INT, D.Producto)) Tienda,
		D.Categoria,
		COUNT(*) Q,
		(SELECT COUNT(*) 
	...
	GROUP BY D.Talla, D.Producto, D.Categoria
	ORDER by D.Talla, D.Producto, D.Categoria
END


EXEC EdiDb.dbo.SP_GetSetInvTodayByBodega 385, 'BOTO2'
...
SELECT * from EdiDB.dbo.WmsProductoExistencia where codUser = 'BOTO2BOT'
SELECT distinct CodProducto from EdiDB.dbo.WmsProductoExistencia where codUser = 'BOTO2BOT'
